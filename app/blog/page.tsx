...
export default function BlogIndexPage() {
  ...
  return (
    <main className="min-h-screen bg-[#0A0A0A] text-[#F5F0E8] pb-24">
      {/* ─── HERO ───────────────────────────────────────────────────────── */}
      <section className="relative overflow-hidden pt-32 pb-20 border-b border-[#C9A84C]/10 bg-[#111111]">
        ...
        <div className="section-container relative z-10 max-w-5xl">
          ...
          <h1 className="font-heading text-4xl font-bold leading-tight md:text-6xl mb-6">
            ...
            <span className="text-[#C9A84C]">& Taxi Guides</span>
          </h1>
          <p className="max-w-2xl text-sm md:text-base leading-relaxed text-[#A1A1A6]">
      ...
      <section className="section-container max-w-5xl py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {publishedPosts.map((post) => (
            <article key={post.slug} className="group rounded-3xl border border-[#C9A84C]/12 bg-[#111111] overflow-hidden hover:border-[#C9A84C]/30 transition-all duration-300 flex flex-col">
              <Link href={`/blog/${post.slug}`} className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-4 left-4">
                  <span className="rounded-full border border-[#C9A84C]/30 bg-black/60 backdrop-blur-md px-3 py-1 text-[0.6rem] font-bold uppercase tracking-wider text-[#C9A84C]">
                    {post.category}
                  </span>
                </div>
              </Link>
              <div className="p-6 flex flex-col flex-1">
                <Link href={`/blog/${post.slug}`} className="block flex-1">
                  <h2 className="font-heading text-lg font-bold text-[#F5F0E8] leading-snug mb-3 group-hover:text-[#C9A84C] transition-colors">
                    {post.title}
                  </h2>
                  <p className="text-xs text-[#A1A1A6] leading-relaxed line-clamp-3 mb-6">
                    {post.excerpt}
                  </p>
                </Link>
                <div className="flex items-center justify-between pt-4 border-t border-[#C9A84C]/10 mt-auto">
                  <div className="flex flex-col gap-1.5 text-[0.6rem] text-[#7C8088] font-medium uppercase tracking-wider">
                  ...
                  <Link href={`/blog/${post.slug}`} className="flex h-8 w-8 items-center justify-center rounded-full bg-[#C9A84C]/10 text-[#C9A84C] group-hover:bg-[#C9A84C] group-hover:text-[#0A0A0A] transition-colors">
