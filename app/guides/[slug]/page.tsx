...
export default async function GuideSinglePage({ params }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#0A0A0A] text-[#F5F0E8] pb-24">
      ...
      />
      {/* ─── HERO ───────────────────────────────────────────────────────── */}
      <section className="relative pt-32 pb-16 overflow-hidden border-b border-[#C9A84C]/10 bg-[#111111]">
        <div className="section-container relative z-10 max-w-3xl">
          <Link href="/guides" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#A1A1A6] hover:text-[#C9A84C] transition-colors mb-8">
            <ChevronRight className="h-4 w-4 rotate-180" /> Back to Knowledge Base
          </Link>
          
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className="flex items-center gap-1.5 rounded-full border border-[#C9A84C]/30 bg-[#C9A84C]/10 px-3 py-1 text-[0.65rem] font-bold uppercase tracking-wider text-[#C9A84C]">
              <Icon className="h-3.5 w-3.5" />
              {guide.category}
            </span>
            {guide.tags.map((tag) => (
              <span key={tag} className="rounded-full border border-[#C9A84C]/10 bg-black/40 px-2.5 py-1 text-[0.6rem] text-[#7C8088] font-medium uppercase tracking-wider">
                {tag}
              </span>
            ))}
          </div>
          ...
          <p className="text-[#A1A1A6] text-lg leading-relaxed max-w-2xl">
            {guide.summary}
          </p>

          <div className="flex items-center gap-6 mt-8 pt-6 border-t border-[#C9A84C]/10 text-xs text-[#7C8088] font-medium uppercase tracking-wider">
        ...
        <div className="prose prose-invert prose-p:leading-relaxed prose-p:text-[#A1A1A6] prose-li:text-[#A1A1A6] prose-headings:font-heading prose-headings:text-[#F5F0E8] prose-a:text-[#C9A84C] max-w-none">
          ...
          <ul className="space-y-8 list-none pl-0">
            {guide.content.map((point, i) => (
              <li key={i} className="flex items-start gap-4 p-6 rounded-2xl bg-[#111111] border border-[#C9A84C]/10">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#C9A84C]/10 border border-[#C9A84C]/30 text-sm font-bold text-[#C9A84C]">
                  {i + 1}
                </span>
                <p className="m-0 pt-1 leading-relaxed text-[#F5F0E8]">{point}</p>
              </li>
            ))}
          </ul>
        </div>
